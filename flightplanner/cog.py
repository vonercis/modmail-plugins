...
class FlightPlannerCog(commands.Cog):
    ...
    def __init__(self, bot, flight_handler):
        ...
        self.processing_lock = {}  # Lock to prevent duplicate processing
    ...
    async def lookup_airport(self, iata_code):
        ...
        if user_id in self.processing_lock:
            print(f"DEBUG: Already processing message for user {user_id}, ignoring")
            return
        
        self.processing_lock[user_id] = True
        print(f"DEBUG: Processing message for user {user_id}, stage: {session.get('stage')}")
        ...
        try:
            ...
            print(f"DEBUG: Finished processing for user {user_id}")
